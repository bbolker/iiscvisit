...
Later we will also load the `glmmADMB` package, but there
are conflicts between some of its methods and `lme4`, so
we'll wait until we really need it.
...
function of fixed effects, including nutrient levels (low *vs.*
high), simulated herbivory (none *vs.* apical meristem damage),
...
*Arabidopsis* is highly selfing, seeds of a single individual
...
str(dat.tf)
@ 
The `X`, `gen`, `rack` and `nutrient` variables are
coded as integers, but we want them to be factors: we coud either
redo the `read.csv` command with `colClasses` set, or
set the variables to factors by hand.

* Use `transform()}, which operates within the data set,
to avoid typing lots of commands like `dat.tf$rack <- factor(dat.tf$rack)`
* At the same time, reorder the clipping variable so that `"unclipped"` is the reference level: could also use `relevel(amd,"unclipped")`
...
* `amd` a factor with levels for no damage or simulated herbivory (apical meristem damage, sometimes called "clipping"
...
Check replication for each genotype (columns) within each population (rows).

```{r reptab}
reptab <- with(dat.tf, table(popu, gen))
## ff <- findMethods("image")
## ff$dgTMatrix
image(Matrix(reptab),
      sub="",
      xlab="Genotype",ylab="Population")
@ 


This mode of inspection is OK for this data set
...
* `plot(reptab)` gives a *mosaic plot*
...
* try the commands `colSums(reptab>0)` and `table(colSums(reptab>0))`
(and the equivalent for `rowSums`) and figure out what they are telling you.
* Compute the range of number of genotypes per treatment combination?

We have only 2-4 populations per region and 2-3 genotypes per population. However, we also have 2-13 replicates per genotype per treatment combination (four unique treatment combinations: 2 levels of nutrients by 2 levels of simulated herbivory). 
Thus, even though this was a reasonably large experiment (625 plants), there were a very small number of replicates with which to estimate variance components at the region level: we are initially going to ignore region (although we could alternatively treat it as a fixed effect, while still including population as a random effect).

We have only two random effects (population, individual), and so
Laplace or Gauss-Hermite Quadrature (GHQ) should suffice, rather than
requiring more complex methods. However, we will have to deal with
overdispersion.

### Look at overall patterns in the data
...
on the log (or $\log(1+x)$) scale.
Box-and-whisker plots are useful summaries and
dot plots  are more detailed, good for seeing if we missed anything:

```{r latticeplots1,fig.width=8}
...
`ggplot` will do just about the same thing.  Its `stat_sum` operator
is useful for displaying duplicated points ...
```{r}
g0 <- ggplot(dat.tf,aes(interaction(nutrient,amd),log(1+total.fruits)))
g0 + geom_boxplot()+facet_wrap(~reg,nrow=1)
g0 + stat_sum() +facet_wrap(~reg,nrow=1)
## sometimes useful too
g0 + coord_flip() + stat_sum() + facet_wrap(~reg,nrow=1)
```

**Exercise** generate these plots and figure out how they work before continuing.
Try conditioning/faceting on population rather than region: for `ggplot`,
take out the `nrow=1` specification.  You could also reorder the
subplots by overall mean fruit set (`reorder`) and/or colour the points according to the region they come from.

### Choose an error distribution; graphical checks
...
Box-and-whisker plot of $\log(\mbox{fruits}+1)$:
```{r bwplots, fig.width=10, fig.height=5}
...
rotating the graph 90 degrees with `coord_flip`
rather than rotating the $x$-axis labels.

```{r fig.keep="none"}
ggplot(dat.tf,aes(x=gna,y=log(1+total.fruits)))+geom_boxplot()+coord_flip()
```

Taking the logarithm of total fruit (or in this case $\log(1+\mbox{fruit})$ to avoid taking the log of zero) should stabilize the variance, but this does not seem to be the case.
The variance actually seems to decline slightly in the largest groups, but it's a bit hard to see.

Calculate the variance  for each genotype $\times$ treatment combination:
```{r checklambda}
grpVarL <- with(dat.tf, tapply(log(1+total.fruits), 
                               list(gna), var) )
summary(grpVarL)
```
There is substantial variation among the sample variances on the transformed data.  In addition to heterogeneous variances across groups, Figure \ref{fig:bw} reveals many zeroes in groups, and some groups with a mean and variance of zero, further suggesting we need a non-normal error distribution, and perhaps something other than a Poisson distribution. 

The boxplots also indicate that the mean ($\lambda$ of the Poisson distribution) is well above 5 (i.e. in general the center of each group is greater than $\log(6)=1.8$), suggesting that PQL may be appropriate, if it is the only option available.
...
summary(grpMeans)
```
The average  by-group $\bar{\lambda}$ is  `r round(mean(grpMeans))}`; the  median is $\approx `r round(median(grpMeans))`$; and at least one group has $\lambda=0$ (i.e., complete absence of fruit set for that genotype $\times$ treatment combination).
...
* as long as the variance is generally greater than the mean, we call 
the data *overdispersed}. Overdispersion comes in various forms:
	* a linear mean-variance relationship with $\mbox{Var} = \phi \mu$ (a line through the origin) with $\phi>1$ is called a *quasi-Poisson* pattern (this term describes the mean-variance relationship, not any particular proability distribution); we can implement it statistically via quasilikelihood \citep{venables_modern_2002} or by using a particular parameterization of the negative binomial distribution (``NB1'' in the terminology of \cite{hardin_generalized_2007})
    * a quadratic relationship, $\mbox{Var}=\mu(1+\alpha \mu)$ or $\mu(1+\mu/k)$, is characteristic of overdispersed data that is driven by underlying heterogeneity among samples, either the negative binomial (gamma-Poisson) or the lognormal-Poisson \citep{elston_analysis_2001}; we will see below how to fit data following these distributions
...
Here are two alternative ways to compute the mean and variance of total fruits by group, using `plyr::ddply} and `reshape2::dcast` (`aggregate`, from base R, can work too):
```{r results="hide"}
ddply(dat.tf,"gna",
...
and negative binomial (linear/quadratic) pattern using `lm`.
(The `-1` in the formulas below
specifies a model with the intercept set to zero.
We have to do some rearranging in the second case,
and use `I()` to specify that we really mean to
...
we also add a non-parametric `loess` fit:
```{r logvarplot}
...
```
The variance-mean relation of Poisson random variables should exhibit a slope of approximately one (dashed line). In contrast, these data reveal a slope of much greater than one (the wedge or cornupia shape is expected for small Poisson samples, but the slope here is much, much greater than 1).
The coloured lines show estimated mean-variance relationships for quasi-Poisson (or negative-binomial ``type I'', \cite{hardin_generalized_2007}) [red]: $V=\phi \mu$;  negative binomial or lognormal-Poisson \citep{elston_analysis_2001} [blue]: $V=\mu(1+\mu/k)$ or  $V=\mu(1+\mu \cdot (\exp(\sigma^2)-1))$.
...
**Exercise**
* compare a simple quadratic fit to the data (i.e., without
...
## Plotting the responses *vs.* treatments

Plot the response by treatment effects, to make sure there are no surprises.
```{r plot1}
stripplot(log(total.fruits+1) ~ amd|popu, dat.tf, 
          ...
          as.table=TRUE)
 ```
Interaction plots of total fruit response to nutrients and clipping, by population. Different lines connect population means of log-transformed data for different nutrient levels.
...
Because we have replicates at the genotype level, we are also able to examine the responses of genotypes.
```{r plot2}
...
```
Interaction plots of total fruit response to clipping, for each genotype (lines and colors). Different panels show different nutrient levels.

`ggplot` versions:  
```{r ggplot,fig.keep="none"}
ggplot(dat.tf,aes(x=amd,y=log(total.fruits+1),colour=factor(nutrient)))+
  geom_point()+
  ## need to use as.numeric(amd) to get lines
  stat_summary(aes(x=amd,group=nutrient),fun.y=mean,geom="line")+
  zmargin+
  facet_wrap(~popu)
ggplot(dat.tf,aes(x=amd,y=log(total.fruits+1),colour=factor(gen)))+
  geom_point()+
  stat_summary(aes(x=amd,group=interaction(nutrient,gen)),
               fun.y=mean,geom="line")+
  ## label_both adds variable name ('nutrient') to facet labels
  facet_grid(.~nutrient,labeller=label_both)
``` 

There appears to be a consistent nutrient effect among genotypes (means in right panel higher than left panel), but the variation increases dramatically, obscuring the clipping effects and any possible interaction.

### Fitting group-wise GLMs
Another general starting approach is to fit GLMs to each group of data separately, equivalent to treating the grouping variables
as fixed effects. This should result in reasonable variation among treatment effects. 
We first fit the models, and then examine the coefficients.
```{r GLMfits}
glm.lis <- lmList(total.fruits~nutrient*amd|gen,data=dat.tf,
                  family="poisson")
```
The versions of `lmList` in the `lme4` and
`nlme` packages are different (only the one in 
 `lme4` has a `family} argument for running GLMs).
 If `find("lmList")` shows a version from `nlme`, use
 `detach("package:nlme")` to get rid of it (you may
...
it was loaded when we `source()`ed the file \verb+glmm_funs.R+.
...
Three genotypes (5, 6, 34) have extreme coefficients.

A mixed model assumes that the underlying random effects are normally distributed, although we shouldn't take these outliers *too* seriously at this point --- 
...
```{r qqmath}
qqmath(glm.lis,id=0.1)
```
Q-Q plots of model coefficients for GLM fits on each genotype.

We see that these extreme coefficients fall far outside a normal error distribution.
...
### Fitting and evaluating the full GLMM
...
although it's really unnecessary in this case because the pattern is so obvious:
...
The *deviance* of the model (in this case, a penalized version of
...
The `printvc} is a utility function that prints the variance-covariance
...
(`amd`) and nutrients, using `amd+nutrient` instead
of `amd*nutrient` in the random effects specification
(here it seems easier to re-do the model rather than 
using `update} to add and subtract terms):
```{r full3,cache=TRUE}
...
like `(1|f)+(0+x|f)` or `(x||f)`; unfortunately, because of the way `lme4} is set up,
...
to overfit (i.e., in order to *not* have perfect $\pm 1$ correlations
...
`amd} or `nutrient}, both, or neither in the model.  
...
observation-level random effects, and then to use `update} to add
...
Now, for example, `update(mp_obs,.~.+(1|gen)+(amd|popu))`
fits the model with intercept random effects at the genotype level
and variation in clipping effects across populations.

One can also model the interaction of categorical
predictors (such as clipping) with a random effect in a slightly more
restricted way, for example coding the `(amd|popu)` term
in the model above as `(1|popu/amd)` instead.  In effect,
 we are treating the different `amd` levels as nested groups
...
to fit models with `amd` or `nutrient` nested within
...
using `printvc`, convince yourself that trying to fit
...
as `mp4`; show that there is non-zero variance estimated at each of these levels
...
Let's check that `mp4` no longer incorporates overdispersion
...
Next we apply a function from `glmm_funs.R` 
that defines a location-scale plot: this is plot \#3 in
the standard `plot.lm` sequence). 
...
```
Location-scale plot, with superimposed loess fit. Zeros in the data are marked in blue.
...
``` 
`glmer` does not return information about the standard errors
...
to admit that the printed representation given by `VarCorr(mp4)`
or `printvc(mp4)` would be just as useful.
...
We use the built-in `dotplot} method for the random effects
extracted from `glmer} fits (i.e. `ranef(model,postVar=TRUE)}),
which returns a list of plots, one for each random effect level in the
model.  (We use a bit of trickery from the `gridExtra} package
...
```
Random effects estimates/predictions for model `mp4`

### Comparing models with different fixed effects

What are the fixed effects?
```{r coefplot1,fig.keep="last"}
coefplot2(mp4)
```
Fixed-effect parameter estimates ($\pm 1$ (thick line) and
$\pm 2$ (thin line) standard error) for the full model}

`glmm_fun.R` defines a convenience function, `daic()`,
that converts the AIC tables
returned by `drop1` (which we will use momentarily)
into $\Delta \textrm{AIC}$ tables.

We use the `drop1` function to assess both the AIC difference
...
`summary` tables for `glmer` fits.

n general you should *not} explore both AIC- and LRT-based comparisons
...
Dropping the `nutrient:amd} interaction gives
...
(in this case using `contr.sum} to change to ``sum to zero'' contrasts)
...
(We don't worry about removing `status} because
...
they are actually available in the `doBy} package
...
If we did `locscaleplot(mp5)} we would see
...
We use `ldply} from the `reshape} package to collapse the list of random effect values into a data
frame (we might have to do something different if there were more than one random effect within each level,
e.g. a model including `(nutrient|gen)}).  The fancy `panel} code in the figure adds a reference
line to the Q-Q plot: see `?qqmath}.
<<reN, fig=TRUE>>=
...
and clipping, as well as the nuisance variables `rack} and `status};
...
layout (with levels [`Nutrient1:Unclipped}, 
`Nutrient1:Clipped}, 
`Nutrient8:Clipped},
`Nutrient8:Unclipped}]); the contrast between 
`Nutrient1:Unclipped} and `Nutrient8:Clipped} would then
...
\textbf{Exercise} 
\begin{itemize}
\item Re-do the analysis with `region} as a fixed effect.
...
Our first alternative is the `glmmADMB} package,
which uses an approach generally similar to `lme4}
although its history and computational approach are very
different.  The main advantage of `glmmADMB} is its
...
load `glmmADMB} (and re-load `coefplot2}):
<<jugglepkgs>>=
...
`glmmADMB} can fit NB1 (`family="nbinom1"}) an NB2
`family="nbinom"}) models; it can also fit zero-inflated
models (with `zeroInflation=TRUE}).
...
@ 
Using `glmmADMB}, you can specify random effects separately
as above, as in `nlme}, or in the same formula along with
the fixed effects, as in `lme4}.

<<>>=
load("Banta_glmmADMB_fits.RData")
@ 

The results are stored in a list called `fits}, with names
`gnb[12][Z]} specifying NB type 1 or 2, without or with
zero-inflation. (The `gnb1B} model will be described below.)

<<>>=
AICtab(fits)
@ 

It looks like NB1 is *much} better than NB2 \ldots but zero-inflation
isn't doing anything.

<<gnbcoefs,fig=TRUE,include=TRUE>>=
...
<<gnbresid,fig=TRUE,include=TRUE>>=
locscaleplot(fits$gnb1)
@ 

By default, `glmmADMB} estimates independent variances for all of the random
effects.
...
you can also use Bayesian approaches \ldots `MCMCglmm}
...
`idh} means to fit the various effects (clipping,
nutrient, and their interaction, as specified by
`amd*nutrient} independently; \verb+~us+ would instead
fit the model with all possible covariances (if you try
this you will see that it doesn't work \ldots). Other
options are possible: see `MCMCglmm} or ask me.
`MCMCglmm} automatically includes an observation-level
...
trace plots *should} look like:

<<MCMCglmmtrace,fig=TRUE,include=TRUE>>=
print(xyplot(as.mcmc(m2$Sol),layout=c(3,3)))
@ 

The variance parameters are an example of what
the trace plots should *not} look like (except
for the `units} variable, which is the observation-level
random effect).
<<MCMCglmmvtrace,fig=TRUE,include=TRUE>>=
...
<<MCMCglmmcoefs,fig=TRUE,include=TRUE>>=
coefplot2(m2)
@ 

<<MCMCglmmvcoefs,fig=TRUE,include=TRUE>>=
