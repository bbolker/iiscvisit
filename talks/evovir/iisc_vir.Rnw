...
\title[Eco-evolutionary virulence dynamics]{Eco-evolutionary virulence of pathogens: \\
...
\begin{description}
\item[People] Arjun Nanda and Dharmini Shah; Christophe Fraser; Marm Kilpatrick; Anson Wong
...
\pause
\item[post-Ewald] more formal tradeoff models, mostly based on $R_0$ optimization.  Adaptive dynamics
\pause
\item[Now] 
\begin{itemize}
...
tmpf <- function(c=5,gamma=1/3) {
    ...
    text("Transmission\nrate",x=0,y=8,xpd=NA)
    axis(side=1,at=c(-2,0:5),labels=c("mu",0:5))
}
tmpf()
Rcol <- "blue"
rcol <- "red"
...
par(xpd=NA)
text(1.5,4.2,expression(beta(alpha)==c*alpha^{1/gamma}),cex=1.2,xpd=NA)
text(3.9,3.3,expression(list(c==2,gamma==2)),col=4)
text(3.6,2.3,expression(list(c==1,gamma==2)),col=1)
text(3.6,1.2,expression(list(c==1,gamma==3)),col=2)
@ 
\end{frame}
...
\begin{itemize}
\item Pathogens with low virulence go unnoticed
  \pause
\item Hosts less resistant to / tolerant of novel parasites 
  \pause
\item High transmission $\to$ frequent coinfection $\to$ selection for virulence
\pause
\item Disease-induced drop in population density decreases selection for virulence \citep{LenskiMay1994}
\end{itemize}
...
offspring production: maximize $R_0$, $\beta N/(\alpha+\mu)$
\pause 
...
{\small
\begin{tabular}{cccl}
\textbf{Pathogen} &  $R_0^*$ & $\alpha^*$ & \textbf{Reference} \\
...
\section[Seasonal disease]{Transient virulence and seasonality}

\subsection{Overview}
\begin{frame}
\frametitle{Seasonality}
\begin{itemize}
\item{Many pathogens fluctuate annually
  \begin{itemize}
  \item{Host contact/aggregation patterns}
  \item{Host (or vector) demography}
  \item{Climatic effects on transmissibility}
  \end{itemize}
}
\item{Fluctuating incidence = fluctuating selection}
\item{Seasonal variation \emph{or} latitudinal variation?}
\end{itemize}
\end{frame}
...
\begin{multicols}{2}
\begin{itemize}
\item Basic Ross-MacDonald vector-host model
\item Simple vector (mosquito) demography
\item No host demography
\item Two pathogen strains
\end{itemize}
\columnbreak
\includegraphics[width=2in]{pix/boxmodel_vec.pdf}
\end{multicols}
\end{frame}

<<get_params>>=
params0L.1 <- transform(params0L,vhratio=50)  ## increase prevalence
params0L.2 <- transform(params0L.1,v=10/11*mu,mu=mu/11) ## modify case mort
@
...
find_rsc <- function(params,target,n,interval=c(0,10)) {
    ...
    uniroot(tmpfun,interval)$root
}
params0L_2str <- nstrainParams(params0L.2,n=2) ## generate a 2-identical-strain version
S1L <- summary(params0L_2str)
rsc <- 1.2
r_target <- S1L[["r"]][1]*rsc
## exploring the
##   scvec <- seq(0,100,length=601)
##   fvec <- sapply(scvec,r_targetfun,params_1mod,n=1,target=r_target)
##   plot(scvec,fvec,type="l")
params_c1 <- set_r(params0L_2str,r=r_target,n=1) ## scale
run_1 <- ode(startfun(params_c1),0:120,w_grad,params_c1)
## boost R0 for strain 1 by 1.5 (multiply betaM)
S2L <- summary(params_c2.1 <- scale_R0(params0L_2str,sc=1.5,n=1))
## Now set the r value for strain 1 back to the baseline value
## (i.e. the r for strain 2)
params_c2 <- set_r(params_c2.1,S2L$r[2],1)
params_c3 <- set_r(set_r(params_c2.1,S2L$r[2],1),
                   S2L$r[2]*rsc,2)
run_2 <- ode(startfun(params_c2),0:120,w_grad,params_c2)
run_3 <- ode(startfun(params_c3),0:120,w_grad,params_c3)
@

%\begin{frame}
%\frametitle{Parameters}

<<partab,results="asis",label="partab",eval=FALSE>>=
sum.list <- lapply(list(params_c1,params_c2,params_c3),summary)
s.tab <- do.call(rbind,lapply(sum.list,unlist))
s.tab[,1:2] <- round(s.tab[,1:2],2)
s.tab[,3:4] <- round(s.tab[,3:4],1)
colnames(s.tab) <- c("$r_1$","$r_2$","$\\rzeroi{1}$","$\\rzeroi{2}$")
rownames(s.tab) <- paste("Case",c("I","II","III"))
latex(s.tab,file="",caption="Summary parameters for simulation cases")
@
%\end{frame}

\begin{frame}
\frametitle{Case I: $r_1>r_2$, equal $\rzero$}
<<plot_case1,warning=FALSE,fig.width=8>>=
gfun <- function(r) {
    do.call(grid.arrange,c(plotfun(r,type="ggplot"),list(nrow=1)))
}
gfun(run_1)
@
\end{frame}
...
\begin{frame}
\frametitle{Case II: $\rzeroi{1}>\rzeroi{2}$, equal $r$}
<<plot_case2,warning=FALSE,fig.width=8>>=
gfun(run_2)
@
\end{frame}

\begin{frame}
\frametitle{Case III: $\rzeroi{1}>\rzeroi{2}$, $r_2>r_1$}
<<plot_case3,warning=FALSE,fig.width=8>>=
gfun(run_3)
@
\end{frame}
...
\begin{frame}
\frametitle{Titer vs infectiousness}
<<vircalc,echo=FALSE>>=
virdat <- read.csv("data/virdat.txt",comment="#")
## For Tiawsirisup 2005 data (where we have numbers infected, but
## transmission percentages don't match exactly -- so `glm()` will
## complain about non-integer numbers of successes), recompute
## the transmission fraction (**to do**: go back to other 
## sources ("Turell altjmh","Dohm","Turell_JME") and get $n$
## values if possible).
virdat <- mutate(virdat,
                 n_inf=round(inf/100*n),
                 n_transmission=round(transmission/100*n),
                 inf=100*n_inf/n,
                 transmission=ifelse(is.na(n),transmission,
                 100*n_transmission/n))
mprof <- with(virdat,seq(max(transmission),100,length=25))/100
glm0 <- glm(transmission/100~titer,weight=n,
            data=virdat,
            family=binomial)
mfit <- function(m) {
    glm(transmission/100~titer,weight=n,
        data=virdat,
        family=binomial(link=logit.2asym(0,1-m)),
        start=coef(glm0))
}
fitList <- lapply(mprof,mfit)
dvec <- sapply(fitList,deviance)
pp <-  interpSpline(mprof,dvec)
bb <- backSpline(pp)
upr.asym <- predict(bb,x=min(dvec)+3.84)$y
@

<<titer_transm>>=
ggplot(virdat,aes(x=titer,y=transmission/100,weight=n))+
    geom_point(aes(colour=source))+
    geom_smooth(method="glm",family=binomial(link=logit.2asym(0,1-mprof[1])),
                fill="blue")+
    geom_smooth(method="loess",colour="red",alpha=0.2)+
    scale_y_continuous(limit=c(0,0.75),oob=squish)+
    labs(y="Transmission probability")+
    annotate(geom="errorbar",x=max(virdat$titer),
             ymin=max(virdat$transmission/100),
             ymax=upr.asym,width=0.25)+
    annotate(geom="point",x=max(virdat$titer),
             y=max(virdat$transmission/100))+
    geom_vline(xintercept=4.5,lty=2)
@
\end{frame}

<<coefs>>=
tv.int <- round(coef(fitList[[1]])["(Intercept)"],1)
tv.sl  <- round(coef(fitList[[1]])["titer"],2)
minpred <- predict(fitList[[1]],newdata=data.frame(titer=4.5),"response")
@

\begin{frame}
\frametitle{Titer curves (American crows)}
<<braultfig>>=
x0 <- read.csv("data/brault_all.csv")
## to predict infection from titer:
coefs <- c(intercept=-14.144,slope=2.01)
prfun <- function(titer)
    with(as.list(coefs),0.6*plogis(intercept+slope*titer))
x0 <- mutate(x0,
             transm=prfun(titer),
             probinf2=transm*surv)
## g1 <- ggplot(x0,aes(day,titer,colour=strain))+geom_line()+
##    scale_colour_discrete(guide=FALSE)+
##    geom_hline(yintercept=4.5,lty=2)
g2 <- ggplot(x0,aes(day,probinf2,colour=strain))+geom_line()+
    ylab("transmission probability")+scale_y_log10()+
    geom_hline(yintercept=minpred,lty=2)
g2
## grid.arrange(g1,g2,nrow=1,widths = unit(c(1,1.5), "null"))
@
\end{frame}
...
<<probhist,eval=FALSE>>=
par(las=1,bty="l")
hist(log10(x0$probinf2),col="gray",breaks=15)
rug(log10(x0$probinf2),col="red")
abline(v=log10(minpred),col="blue",lwd=2)
@


<<titersum>>=
minvir <- 3e-3
x0sum <- ddply(x0,"strain",
               summarise,
               source=head(source,1),
               a2 = mean(probinf2[probinf2>minvir]),
               infper=sum(probinf2>minvir),
               casemort=1-tail(surv,1))
@

%\begin{frame}
%\frametitle{Transmission vs clearance}
<<virplot1B,warning=FALSE,eval=FALSE>>=
(g2 <- ggplot(x0sum,aes(1/infper,a2))+
 geom_text(aes(colour=source,label=strain))+
 scale_x_continuous(limits=c(0,1))+
 stat_function(fun=function(x) 0.7*x^0.5)+
    labs(x="Clearance rate (1/infectious period)",
...
<<sparrow>>=
spdat <- read.csv("data/brault2011_sparrow_viremia.csv")
## ggplot(spdat,aes(day,titer,colour=strain))+geom_line()+geom_point()
spdat <- mutate(spdat,
             transm=prfun(titer),
             probinf2=transm*surv)
spsum <- ddply(spdat,"strain",
               summarise,
               a2 = mean(probinf2[probinf2>minvir]),
               infper=sum(probinf2>minvir),
               casemort=1-tail(surv,1))
allsum <- rbind(data.frame(species="sparrow",source="Brault2007",
                           spsum),
                data.frame(species="amcrow",x0sum))
@


\begin{frame}
\frametitle{Transmission vs clearance for WNV}
<<sparrow_crow2,warning=FALSE,message=FALSE>>=
## http://stackoverflow.com/questions/8197559/emulate-ggplot2-default-color-palette
gg_color_hue <- function(n) {
  hues = seq(15, 375, length=n+1)
  hcl(h=hues, l=65, c=100)[1:n]
}
gcols <- gg_color_hue(2)
(g3 <- ggplot(allsum,aes(1/infper,a2))+
 geom_text(aes(colour=species,label=strain))+
 scale_x_continuous(limits=c(0,1))+
## stat_function(fun=function(x) 0.7*x^0.5,colour=gcols[2],lty=3)+
## stat_function(fun=function(x) 0.3*x^0.5,colour=gcols[1],lty=3)+
 labs(x="Clearance rate (1/infectious period)",
      y="Average transmission rate")+
 scale_colour_brewer(palette="Set1")+
 scale_x_continuous(limits=c(0,1))+
...
## baseParams = transforms(params_robinX,...)
...
par(las=1,bty="l")
with(rgrid,contour(clearance,beta,r,
                   xlab="Clearance rate (1/infectious period)",
                   ylab="Average transmission rate"))
with(r0grid,contour(clearance,beta,r0,levels=c(1,2,5,10),
                    col="red",lty=2,add=TRUE))
@


%\begin{frame}
%\frametitle{lots of species}
<<komar2,warning=FALSE,eval=FALSE>>=
Kdat <- read.csv("data/Komar_2003.csv",comment="#",as.is=TRUE)
Kdat[-1] <- lapply(Kdat[-1],as.numeric)
mdat <- melt(Kdat,id.vars=1:2)
vars <- colsplit(mdat$variable,"_",names=c("day","w"))
vars$day <- as.numeric(gsub("day","",vars$day))
vars <- na.omit(vars) ## drop duration stuff
mdat2 <- cbind(mdat[1:525,c("Species","n")],vars,value=mdat$value[1:525])
mdat3 <- dcast(mdat2,Species+n+day~w)
## ggplot(mdat3,aes(x=day,y=est))+geom_line(aes(colour=Species))
kdat2 <- mutate(mdat3,
                transm=prfun(est),
                probinf2=transm)
ksum <- ddply(kdat2,"Species",
              summarise,
              a2 = mean(probinf2[probinf2>minvir],na.rm=TRUE),
              infper=sum(probinf2>minvir,na.rm=TRUE),
              casemort=NA)
ksum <- subset(data.frame(rename(ksum,c(Species="species")),
                   source="Komar2003",strain="NY99"))
ksum2 <- transform(ksum,
                   species=factor(ifelse(!species %in% c("American Crow",
                   "American Robin","House Sparrow","Fish Crow"),
                   "other",species)))
levels(ksum2$species) <- c("amcrow","robin","fishcrow","sparrow","other")
allsum2 <- rbind(allsum,ksum2)
allsum2$strain2 <- with(allsum2,ifelse(strain=="NY99","NY99","other"))
ggplot(subset(allsum2,strain=="NY99"),
       aes(x=1/infper,y=a2,colour=species,shape=source))+
    scale_colour_manual(values=c("red","blue","purple","cyan","gray"))+
    geom_point(size=4)+
    geom_point(data=subset(allsum2,strain!="NY99"),colour="gray")+
     labs(x="Clearance rate (1/infectious period)",
      y="Average transmission rate")
@
%\end{frame}

\section[Theory vs. data]{More on theory vs. data}

\subsection{Tradeoff curves}

\begin{frame}
\frametitle{Estimating tradeoff curves}
\begin{itemize}
\item Usually \emph{assume} a tradeoff between virulence and transmission
\item Positive \emph{correlation}
virulence and transmissibility 
(or proxies) known from many systems \citepx{LipsitchMoxon1997}
\item \colemph{shape} of tradeoff curves is largely unknown
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Malaria \citepx{MackinnonRead1999a,Paul+2004}}
\begin{center}
<<maltrade,fig.width=10,fig.height=5>>=
par(mfrow=c(1,2))
...
\begin{frame}
  \frametitle{\emph{Pasteuria ramosa} \citepx{Jensen+2006}}
<<jensenfig>>=
## par(las=1)
par(cex=1.5,lwd=2,bty="l",yaxs="i",las=1,mgp=c(2,0.5,0),
    mar=c(4,4,1,1))
...
%\begin{frame}
%\frametitle{Tradeoff curve: \emph{Pasteuria ramosa} \citepx{Jensen+2006}}
<<eval=FALSE,echo=FALSE>>=
x = read.table("data/jensen-fig.dat")
names(x) =c("Days","Spores")
x$Days = round(x$Days)
x$Spores = pmax(0,x$Spores)
plot(x$Days,x$Spores)
g1 = glm(Spores~poly(Days,2),family=quasipoisson,data=x)
newdays = seq(min(x$Days),max(x$Days),length.out=100)
vals = predict(g1,newdata=data.frame(Days=newdays),type="response")
lines(newdays,vals)
g0 = glm(Spores~Days,family=quasipoisson,data=x)
## anova(g0,g1,test="F")
newdays2 = 1/seq(0.001,0.05,length=100)
vals2 = predict(g1,newdata=data.frame(Days=newdays2),type="response",se.fit=TRUE)
## mean longevity APPROX 130 days
meanmu = 1/130
newalpha1 = (1/x$Days)/meanmu
@ 
<<eval=FALSE,fig.height=5>>=
par(mar=c(5,4,3,1),cex=1.5,lwd=1.5,las=1,bty="l",
    mgp=c(2.75,1,0))
plot(newalpha1,x$Spores/x$Days,xlim=c(0,max(newalpha1)),
     xlab="Scaled virulence",ylab=expression(paste("Spores/day ",({}%*%10^6))))
newalpha=(1/newdays2)/meanmu
lines(newalpha,vals2$fit/newdays2)
matlines(newalpha,(vals2$fit+outer(vals2$se.fit,c(-1,1)))/newdays2,lty=2,
         col=1)
...
\begin{frame}
  \frametitle{HIV \citepx{Fraser+2007}}
...
\begin{frame}
  \frametitle{HIV dynamics \citepx{shirreff_transmission_2011}}
...
\begin{multicols}{2}
\begin{itemize}
\item Theory: spatial structure should select for decreased virulence
\item Experiment: viscosity decreases \colemph{infectivity}
in \emph{Plodia} \citep{BootsMealor2007}
\item Are we ready for space?
\end{itemize}

\columnbreak
\includegraphics[width=0.5\textwidth]{pix/BootsMealor.png}
...
%   $\alpha(0)= \alpha^*$
\item Eco-evolutionary dynamics of virulence are still plausible
\citepx{alizon_virulence_2009,luo_navigating_2013}
%but parameters are very poorly known
\pause
\item Sensitive to genetic variance and shape of tradeoff curve
