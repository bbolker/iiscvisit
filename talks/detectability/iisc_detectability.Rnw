...
\institute{Math Bio Research Seminar}
\date{3 October 2014}
...
<<fig.width=8,fig.height=3,out.width="\\textwidth">>=
...
\begin{frame}
  \frametitle{Cross-citation study}
  \begin{itemize}
  \item who cares about math biology?
  \item more specifically, what is the information flow \\
    from MB to bio (or math) and \emph{vice versa}?
  \item extract information from ISI Journal Citation Report \\
    (thanks to Aaron Berk)
  \item find top 100 cited/citing journals for: \\
    (Bull Math Biol, J Theor Biol, Theor Popul Biol,
    J Math Biol, Math Biosci, PLoS Comput Biol)
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Ordination of (1/(1+avg cites))}
<<nmds1,fig.width=8,out.width="\\textwidth">>=
load("metaMDSplot.RData")
g1 <- ggplot(mmdat2,aes(MDS1,MDS2,label=journal))+
              labs(x="NMDS axis 1",y="NMDS axis 2")+
              theme(axis.text.x=element_blank(),
                   axis.text.y=element_blank())
g1+geom_text(aes(colour=cat3),size=3)+
    scale_colour_brewer(palette="Dark2")
@ 
\end{frame}

\begin{frame}
  \frametitle{Biology + math biology only}
<<nmds2,fig.width=8,out.width="\\textwidth">>=
colvec <- c(colorRampPalette(c("blue","green"))(14),
            "cyan","purple","orange")
g1 %+% subset(mmdat2,cat4!="other") + 
    geom_text(aes(colour=cat4),size=3)+
    scale_colour_manual(name="",values=colvec)
@ 
\end{frame}
